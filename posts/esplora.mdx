---
title: Esplora Meta Data.
description: "Esplora HTTP API."
date: Sept 4 2022
---
...
[Bitcoin](https://blockstream.info/api/) blockstream.info/api/
[Bitcoin Testnet](https://blockstream.info/testnet/api/) testnet/api

For example:
```jsx
$ curl [curl](https://blockstream.info/api/blocks/tip/hash) api/blocks/tip/hash
You can also self-host the Esplora API server, which provides better privacy and security.
...
Available fields: id, height, version, timestamp, mediantime, bits, nonce, merkle_root, tx_count, size, weight, and previousblockhash. Elements-based chains have an additional proof field. See block format for more details.
...
If /decimal is specified, returns the supply as a decimal according to the asset's divisibility. Otherwise, returned in base units.
